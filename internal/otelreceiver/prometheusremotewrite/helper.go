...
func addResourceTargetInfo(resource pcommon.Resource, settings Settings, ts pcommon.Timestamp, tsMap map[string]*prompb.TimeSeries) {
...
func convertTimeStamp(ts pcommon.Timestamp) int64 {
	return ts.AsTime().UnixNano() / (int64(time.Millisecond) / int64(time.Nanosecond))
}
