...
func (oq *OrganizationQuery) loadRepositories(ctx context.Context, query *RepositoryQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Repository)) error {
	...
	for _, n := range neighbors {
		fk := n.organization_repositories
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_repositories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_repositories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
